/*
  # Schema inicial do FJJ Brasileirão

  1. Novas Tabelas
    - `championships` - Armazena informações dos campeonatos
    - `teams` - Times participantes
    - `players` - Jogadores
    - `matches` - Partidas
    - `match_scorers` - Gols marcados nas partidas
    - `transfers` - Transferências de jogadores
    - `championship_history` - Histórico de campeões

  2. Security
    - RLS habilitado em todas as tabelas
    - Políticas de acesso público para leitura
    - Políticas restritas para escrita (apenas usuários autenticados)
*/
